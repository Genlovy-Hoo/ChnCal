...
import pandas as pd
try:
    from .lunar import sol2gz, get_tgdz_hour, lun2sol, sol2lun, hour2dz, SX, DZ
    ...
    from chncal.lunar import sol2gz, get_tgdz_hour, lun2sol, sol2lun, hour2dz, SX, DZ
...
def get_age_by_shuxiang(shuxiang, year: int = None, return_n: int = 10):
    """根据属性获取可能年龄"""
    assert isinstance(shuxiang, str) and (shuxiang in SX or shuxiang in DZ)
    base_year = 2022
    if shuxiang in SX:
        SX_ = SX
        base_sx = '虎'
    ...
    res = {}
    n = 0
    year, idx = base_year, SX_.index(base_sx)
    while n < return_n:
        if SX_[idx] == shuxiang:
            age = now_year - year
            if age >= 0:
